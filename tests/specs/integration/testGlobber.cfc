component extends="tests.resources.ModuleIntegrationSpec" appMapping="/app" {
    ...
    function run() {
        describe( "The Globber fluent API", function() {
			...
			it( title="Can match a Windows drive root", skip=()=>!directoryExists('C:/'), body=function() {
				var results = globber
					.setPattern( 'C:' )
					.matches();
				...
				expect( results.directory ).toBe( 'C:' );
				expect( results.type ).toBe( 'Dir' );

			} );

			it( title="Can list a Windows drive root", skip=()=>!directoryExists('C:/'), body=function() {
				var results = globber
					.setPattern( 'C:/*' )
					.matches();

				expect( results.len() ).toBeGT( 1 );
				expect( results ).toInclude( 'C:\Windows\' );
